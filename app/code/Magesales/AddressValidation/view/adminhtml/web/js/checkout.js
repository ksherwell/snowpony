...
function ($, modal, $t){
    ...
    $.widget('mage.AddressValidationCheckout', {
        options: {
            urlValidation: "",
        ...
        disableNextButton: function(message){
            ...
            $('.magesales-address-validation-error-message').remove();
            $('button.save').parent()
                .append('<div style="clear:both"></div><label generated="true" class="magesales-address-validation-error-message mage-error">' +
                $t(message) +
                '</label>');
        }
    });

    return $.mage.AddressValidationCheckout;
});