    ...
    public function run(&$results, $fix, $productId = null)
    {
        ...
        foreach($this->getStocks() as $stock)
        {
            if (!in_array($stock['website_id'], $websiteIds))
                $results['unconsistant_stock']['items'][] = 'Stock #'.$stock->getId().' refers to unexisting website #'.$stock['website_id'].' (this error can not be fixed automatically, a DB intervention is required)';
